...
function findLongestWord( array_words ){
	...
	for (i = 0; i < array_words.length; i++){
		currentLenght = array_words[i].length
		if (value <= currentLenght) {
			value = currentLenght;
		};
	}
	return value;
...
var array_lengthss = []
array_lengthss.push( findLongestWord( ["Batman", "Superman", "Spiderman", "Hulk"] ) );
array_lengthss.push( findLongestWord( ["Car", "Bike", "Boat", "Aeroplane", "Train"] ) );
array_lengthss.push( findLongestWord( ["Soccer", "Basketball", "Tennis", "Golf", "Rowing", "Horse Riding"] ) );


console.log( "These are the longest lengths of each group: " + array_lengthss );
console.log( calculateAverage( array_lengthss ) === 10 && "The average of longest lengths is: " + calculateAverage( array_lengthss ));
...
function arrangeElements( array )
{
	...
	var arrageArray = tmp_array.concat(array);
	return arrageArray;
}
...
function beautifyLetters( array )
{
	...
	for (i = 0; i < array.length; i++){
		if (isNaN(array[i]) === false){
			beautifyArray.push(array[i]);
		}else if ((array[i] == "a") || (array[i] == "e") || (array[i] == "i") || (array[i] == "o") || (array[i] == "u") || (array[i] == "A") || (array[i] == "E") || (array[i] == "I") || (array[i] == "O") || (array[i] == "U")) {
			beautifyArray.push(array[i].toUpperCase());

		}else{
				beautifyArray.push(array[i].toLowerCase());
		} 
	}
	return beautifyArray;
}
...
function beautifyNumbers( array )
{
    ...
    function reduce_number( num ) {
        var nArray = num.toString().split('');
        var j;
        var sum = 0;

        for (j = 0; j < nArray.length; j++) {
            sum += parseInt(nArray[j]);
        }
...
function sortArray( array )
{
	...
	sorted = numArray.concat(charArray);
